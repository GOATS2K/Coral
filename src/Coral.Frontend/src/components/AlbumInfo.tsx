...
export default function AlbumInfo({ album }: AlbumInfoProps) {
  ...
  const getGenre = () => {
    let uniqueGenres = Array.from(new Set(album.tracks?.map((a) => a.genre?.name)));
  ...
  const getDuration = () => {
    let totalDurationInSeconds = album.tracks?.map((t) => t.durationInSeconds).reduce((a, b) => a + b, 0);
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.metadataWrapper}>
        <div className={styles.metadata}>
          <Text fw={700} fz={32}>{album.name}</Text>
          <Text fz={20}>{getAlbumArtists()}</Text>
        </div>
        <div className={styles.attributes}>
          <Text c={"dimmed"} fz={16} className={styles.attribute}>{album.releaseYear}</Text>
          <Text c={"dimmed"} fz={16} className={styles.attribute}>{album.tracks?.length} tracks</Text>
          <Text c={"dimmed"} fz={16} className={styles.attribute}>{getDuration()}</Text>
          <Text c={"dimmed"} fz={16} className={styles.attribute}>{getGenre()}</Text>
        </div>
      </div>
    </div>
  )
